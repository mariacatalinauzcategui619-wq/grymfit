...
def cargar_datos_drive():
    ...
    if not os.path.exists(ruta_json):
        st.error("No se encontró 'credentials.json' en la carpeta Gym.")
        st.stop()
    ...
    try:
        ...
        query = "name contains 'Planificador General' and mimeType='application/vnd.google-apps.spreadsheet'"
        results = drive_service.files().list(q=query, fields="files(id, name)").execute()
        files = results.get('files', [])

        if not files:
            st.error("No se encontró el archivo 'Planificador General' en Google Drive.")
            st.stop()
